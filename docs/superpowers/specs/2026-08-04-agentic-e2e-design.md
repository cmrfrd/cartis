...
1. **The app under test:** the cartis dev server spawned with
   ...
   only `src/` change). The scratch root is seeded by copying the scenario's
   fixture dir. The app's own agent spawns real opencode as usual;
   `REPLICATE_API_TOKEN` is STRIPPED from the child env so art uses the stub.
2. **The driver:** a second opencode server via the same `createOpencode`
   ...
   - its **cwd is a scratch driver dir**, not the repo — opencode sessions
     ...
     staged into this dir. Driver permissions are set permissive in config so
     tool calls don't block on prompts.
3. **Scenarios, sequentially** (shared port 5173): each is one fresh driver
...
kill the dev server. A per-scenario timeout (`timeoutMin`) aborts the session
...
http://localhost:5173 driving a browser via the chrome-devtools tools; act
...
CI integration (manual trigger only), scripted/Playwright tests (separate
future track), the fake-agent bridge stub, direct-CDP verification, visual
pixel-diff parity, parallel scenarios, Windows/Linux support beyond what
`bunx chrome-devtools-mcp` provides.
