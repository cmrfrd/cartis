...
// The parser-era patch validators (schemaFromFields, decodePatchLenient)
// lived here until the pi migration: tool arguments are now validated by the
// provider itself via the per-turn typebox schemas in
// src/server/pi/cardTools.ts — the constraints travel as FieldSummary and
// become the tool's own parameter schema.
